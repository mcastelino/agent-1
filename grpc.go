...
// req.FileSize is 2MB and req.Content contains the first half of the file, in the seconds call req.Offset is 1MB,
// req.FileSize is 2MB and req.Content contains the second half of the file. For security reason all write operations
...
func (a *agentGRPC) CopyFile(ctx context.Context, req *pb.CopyFileRequest) (*gpb.Empty, error) {
	...
	if err := os.MkdirAll(filepath.Dir(req.Path), os.FileMode(req.DirMode)); err != nil {
		return emptyResp, err
	}
	...
	if _, err := tmpFile.WriteAt(req.Content, req.Offset); err != nil {
		tmpFile.Close()
		return emptyResp, err
	}
	tmpFile.Close()
